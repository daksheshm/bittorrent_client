...
class PeerInfo:
	...
	def to_compact(self) -> bytes:
		"""Return the peer info in compact form. The first 4 bytes are the ip address and the last 2 bytes are the port number."""
		# Transform IP address string into four bytes
		ip_bytes = self.ip.split('.')
		ip_bytes = [int(b) for b in ip_bytes]
		return bytes(ip_bytes) + self.port.to_bytes(2, byteorder='big')
