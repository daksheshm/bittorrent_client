...
def decode_extract(s, extract_key: bytes = None):
    """
    ...
    def decode_first(s):
        ...
        if s.startswith(b"i"):
            ...
            while not rest.startswith(b"e"):
                ...
                if rest_w_val and not val_bc:
                    val_bc = rest_w_val[:len(rest_w_val)-len(rest)]
                if elem == extract_key:
                    rest_w_val = rest
                l.append(elem)
            rest = rest[1:]
...
def decode(s):
    ret, val_bc = decode_extract(s)
    return ret

''' Gets the bencoded value mapped to a specified dictionary key anywhere in the string '''
def get_bencoded_val(s, key: bytes):
    ret, val_bc = decode_extract(s, key)
    return val_bc
